...
describe("Activity Log API", () => {
  ...
  describe("Authentication", () => {
    test("unauthenticated user should not access activity logs", async () => {
    ...
    test("unauthenticated user should not access activity log by id", async () => {
      const response = await request(app)
        .get("/api/activity-logs/507f1f77bcf86cd799439011");

      expect(response.status).toBe(401);
    });

    test("customer should not access activity logs", async () => {
      const response = await request(app)
        .get("/api/activity-logs")
        .set(
          "Authorization",
    ...
    test("customer should not access activity log by id", async () => {
      const response = await request(app)
        .get(
  ...
  describe("Validation", () => {
    ...
    test("should reject invalid activity log id", async () => {
      const response = await request(app)
        .get("/api/activity-logs/not-a-valid-id")
        .set(
          "Authorization",
          `Bearer ${adminToken}`
        );
  ...
  describe("Activity Log Retrieval", () => {
    test("GET /api/activity-logs should return activity logs", async () => {
      ...
      expect(Array.isArray(response.body.logs)).toBe(
        true
      );
    ...
    test("GET /api/activity-logs/:id should return 404 for valid but nonexistent id", async () => {
      const response = await request(app)
        .get(
          "/api/activity-logs/507f1f77bcf86cd799439011"
        )
    ...
    test("GET /api/activity-logs/:id should return an existing log", async () => {
      const log = await ActivityLog.create({
        userId: adminUserId,
      ...
      expect(response.body.log._id).toBe(
        log._id.toString()
      );

      expect(response.body.log.action).toBe(
        "GET_BY_ID_TEST"
      );

      expect(response.body.log.entity).toBe(
        "ActivityLog"
      );
    });
  });
  ...
  describe("Activity Log Service / Database", () => {
    test("should be able to create and retrieve an activity log", async () => {
      ...
      const foundLog = await ActivityLog.findById(
        createdLog._id
      ).lean();
      ...
      expect(foundLog.entityId).toBe("test-123");
