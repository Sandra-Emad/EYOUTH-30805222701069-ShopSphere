...
describe("Activity Log API Integration", () => {
  ...
  beforeAll(async () => {
    if (!mongoUri) {
      throw new Error(
        "MongoDB connection string is not configured. Set MONGODB_TEST_URI, MONGODB_URI, MONGO_URI, or MONGO_URL."
      );
    }
    ...
    /*
     * Create admin user through existing auth API
     */
    ...
    /*
     * Make sure JWT contains ADMIN role.
     */
  ...
  describe("Activity Log Service / Database", () => {
    test(
      ...
      async () => {
        ...
        const retrieved =
          await ActivityLog.findById(log._id);

        expect(retrieved).toBeDefined();
        expect(retrieved).not.toBeNull();
        expect(retrieved.action).toBe("TEST");
